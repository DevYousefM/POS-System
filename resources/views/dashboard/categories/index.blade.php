...
@section('content')
    <section class="content-wrapper">
        ...
        <section class="content-header">
            <div class="container-fluid">
                <form class="row flex-row-reverse" action="{{route('dash.categories.index')}}" method="get">
                    <div class="col-md-4 col-6">
                        <input type="text" name="search" value="{{request()->search}}"
                               placeholder="@lang("site.searchAboutCa")" class="form-control">
                    </div>
                    <div class="d-flex">
                        @if(auth()->user()->hasPermission("categories_create"))
                            <a class="btn btn-info" href="{{route("dash.categories.create")}}">@lang("site.add") <i
                        ...
                        <tr>
                            ...
                            <td>
                                <a class="btn btn-info "
                                   href="{{route("dash.products.index",["category"=>$category->id])}}">@lang("site.related_products")</a>
                            ...
                            <td>
                                <div class="d-flex">
                                    ...
                                    <a href="{{ $edit ? route("dash.categories.edit",$category) : "#" }}"
                                       class="btn btn-info mr-1 {{$edit ? "" : 'disabled'}}">
                                        {{--                                            @lang("site.edit")--}}
                                        <i class="fa fa-edit"></i>
                                    </a>
                                    <form class="formDelete"
                                          onsubmit="return confirm('@lang("site.delete_confirm_category")');"
                                          action="{{$delete ? route("dash.categories.destroy",$category) : "#" }}"
                                          method="post">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
